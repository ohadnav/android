...
public class CameraUtilTest {
    ...
    public void toByteArray() throws Exception {
        ...
        byte[] expected = AssetsReaderUtil.readAsBytes(mTheaterActivityTestRule.getActivity(),
                                                       CameraTestUtil.BITMAP_1x1_PATH);
        byte[] actual = CameraUtil
                .toByteArray(CameraTestUtil.bitmapBytesToMockedImage(expected, 0));
        assertArrayEquals(expected, actual);
    }

    @Test
    public void compareEquals() throws Exception {
        Image image = CameraTestUtil.bitmapBytesToMockedImage(AssetsReaderUtil.readAsBytes(
                mTheaterActivityTestRule.getActivity(), CameraTestUtil.BITMAP_1x1_PATH), 0);
        assertTrue(CameraUtil.compare(image, image));
    }

    @Test
    public void compareNotEquals() throws Exception {
        byte[] source = AssetsReaderUtil.readAsBytes(mTheaterActivityTestRule.getActivity(),
                                                     CameraTestUtil.BITMAP_1x1_PATH);
        ...
        Image image2 = CameraTestUtil
                .bitmapBytesToMockedImage(CameraTestUtil.bitmapToByteArray(bitmap), 0);
