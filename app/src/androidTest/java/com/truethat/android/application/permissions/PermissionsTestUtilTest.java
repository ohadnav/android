...
public class PermissionsTestUtilTest {
  ...
  @Test public void revokeAllPermissions() throws Exception {
    ...
    assertNotEquals(PackageManager.PERMISSION_GRANTED,
        ActivityCompat.checkSelfPermission(activityTestRule.getActivity(), Manifest.permission.CAMERA));
    assertNotEquals(PackageManager.PERMISSION_GRANTED,
        ActivityCompat.checkSelfPermission(activityTestRule.getActivity(), Manifest.permission.ACCESS_FINE_LOCATION));
    // Ask for permission
    ActivityCompat.requestPermissions(activityTestRule.getActivity(),
        new String[] { Manifest.permission.CAMERA, Manifest.permission.ACCESS_FINE_LOCATION }, sRequestCode);
  ...
  @Test public void revokePermission() throws Exception {
    ...
    assertNotEquals(PackageManager.PERMISSION_GRANTED,
        ActivityCompat.checkSelfPermission(activityTestRule.getActivity(), Manifest.permission.CAMERA));
    // Ask for permission
    ActivityCompat.requestPermissions(activityTestRule.getActivity(), new String[] { Manifest.permission.CAMERA },
        sRequestCode);
    UiObject2 allowButton = mDevice.wait(PermissionsTestUtil.ALLOW_SEARCH_CONDITION, 100);
    MatcherAssert.assertThat(allowButton.isEnabled(), Is.is(true));
    allowButton.click();
